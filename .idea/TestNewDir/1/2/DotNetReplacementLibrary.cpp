...
namespace Replacement
{
	...
	int DotNetReplacementLibrary::ConvertParamsToTypedArrays(Object* params [],
		void** &paramValues, ParameterType* &paramTypes)
	{
		...
		for (int i=startIndex; i<params->Count; i++)
		{
			try
			{
				...
				const wchar_t * wszParamStr;
				if (obj == NULL)
					wszParamStr = PtrToStringChars("null");
				else
	...
	bool DotNetReplacementLibrary::RunStandardTestsAndGetResults(OriginalMethodCaller* ofCaller, String* category,
		String* functionName, Object* params [], Type* returnType, Object* &returnValue, Exception* &exception)
	{
		IncrementCallLevel();
	...
	Exception* DotNetReplacementLibrary::ConvertExceptionStringToObject(String* str)
	{
		...
		return dynamic_cast<Exception*>(constructor->Invoke(new Object* [0]));
	}
	...
	{
		const wchar_t * wszCategory = PtrToStringChars(category);
		const wchar_t * wszFunctionName = PtrToStringChars(functionName);
		...
		void** paramValues, ParameterType* paramTypes, Exception* exception, Object* returnValue)
	{
		...
		if (returnValue != NULL)
		{
			const wchar_t * wszReturnValue = PtrToStringChars(returnValue->ToString());
		...
		if (exception != NULL)
		{
			const wchar_t * wszException = PtrToStringChars(exception->GetType()->Name);
			sException = wszException;
		}

		const wchar_t * wszOriginalDLLName = PtrToStringChars(originalDllName);
